...
User.prototype.validate = function () {
  return new Promise(async (resolve, reject) => {
    ...
    if (this.data.password.length > 50) {
      this.errors.push("Password cannot be exceed 50 characters.");
    }
    if (this.data.username.length > 0 && this.data.username.length < 3) {
      this.errors.push("Username must be at least 3 characters.");
    }
    if (this.data.username.length > 30) {
      this.errors.push("Username cannot be exceed 30 characters.");
    }
    ...
    if (
      ...
      let usernameExits = await usersCollection.findOne({
    ...
    //Only if username is valid then check if it's already taken
    if (validator.isEmail(this.data.email)) {
      let emailExits = await usersCollection.findOne({
